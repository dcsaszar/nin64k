...
func (c *CPU6502) branch(cond bool) int {
	oldPC := c.PC
	offset := int8(c.Read(c.PC)); c.PC++
	if cond {
		c.PC = uint16(int32(c.PC) + int32(offset))
		// +1 cycle for taken branch, +1 more if page boundary crossed
		pageCross := 0
		if (oldPC&0xFF00) != (c.PC&0xFF00) { pageCross = 1 }
		return 1 + pageCross
	}
	return 0
}
...
func (c *CPU6502) Step() bool {
	...
	case 0x10: extraCycles = c.branch(!c.getFlag(FlagN))
	case 0x30: extraCycles = c.branch(c.getFlag(FlagN))
	case 0x50: extraCycles = c.branch(!c.getFlag(FlagV))
	case 0x70: extraCycles = c.branch(c.getFlag(FlagV))
	case 0x90: extraCycles = c.branch(!c.getFlag(FlagC))
	case 0xB0: extraCycles = c.branch(c.getFlag(FlagC))
	case 0xD0: extraCycles = c.branch(!c.getFlag(FlagZ))
	case 0xF0: extraCycles = c.branch(c.getFlag(FlagZ))
	case 0x18:
	...
	if cycles, ok := instrCycles[opcode]; ok {
		c.Cycles += uint64(cycles + extraCycles)
	} else {
		c.Cycles++ // fallback for unknown instructions
	}
	// Note: Page boundary crossings for AbsX/AbsY/IndY are already accounted for in the base cycle table
	// (using worst-case timing). Proper detection would require checking page crossing for each addressing mode.
	return false
}
...
// 6502 cycle counts per instruction (not accounting for page boundary crossing)
var instrCycles = map[byte]int{
