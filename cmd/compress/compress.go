...
func main() {
	...
	// Output checksums and sizes for selftest
	fmt.Println("\nSelftest checksums (16-bit additive):")
	fmt.Println("selftest_checksums:")
	var checksumInc strings.Builder
	checksumInc.WriteString("; Auto-generated by compress - do not edit\n")
	checksumInc.WriteString("selftest_checksums:\n")
	for song := 1; song <= 9; song++ {
		var csum uint16
		for _, b := range songs[song] {
			csum += uint16(b)
		}
		line := fmt.Sprintf("        .word   $%04X               ; Song %d\n", csum, song)
		fmt.Print(line)
		checksumInc.WriteString(line)
	}
	checksumInc.WriteString("\nselftest_sizes:\n")
	fmt.Println("\nSelftest sizes:")
	fmt.Println("selftest_sizes:")
	for song := 1; song <= 9; song++ {
		line := fmt.Sprintf("        .word   %-5d               ; Song %d\n", len(songs[song]), song)
		fmt.Print(line)
		checksumInc.WriteString(line)
	}
	fmt.Println("\nStream checksum:")
	...
	checksumInc.WriteString(fmt.Sprintf("\nselftest_stream_csum:  .word $%04X\n", streamCsum))

	// Write to generated file
	if err := os.WriteFile("generated/selftest_checksums.inc", []byte(checksumInc.String()), 0644); err != nil {
		fmt.Printf("Warning: could not write selftest_checksums.inc: %v\n", err)
	}
}
